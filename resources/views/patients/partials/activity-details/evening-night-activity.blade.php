@php $activity_form = getShiftLogField(3);
	 ...
	 $log_created_by = \App\Models\PatientActivity::where(['log_id' => $log->id, 'shift_id' => 3])->first();
@endphp
...
@endphp
	...
	<div class="d-flex justify-content-between align-items-center flex-wrap mb-5">
		...
		<span class="text-muted">Reporter: {{ $evening_time != '' ? ($log_created_by->added_by ? $log_created_by->added_by->name : '-') : '' }}</span>
		<span class="text-muted">Reporting Time: {{ $evening_time }}</span>
	</div>
	@endif
@else
	<div>
