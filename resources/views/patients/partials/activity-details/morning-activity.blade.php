...
foreach ($activity_form[0]['subchilds'] as $key => $subchild) {
	 ...
	 $log_created_by = \App\Models\PatientActivity::where(['log_id' => $log->id, 'shift_id' => 1])->first();
@endphp
@if($morning_time != '')
@php
...
@endphp
	...
	<div class="d-flex justify-content-between align-items-center flex-wrap mb-5">
		...
		<span class="text-muted">Reporter: {{ $log_created_by != null ? $log_created_by->added_by->name : '-' }} - {{$log_created_by->added_by->role_id == 2 ? 'Admin' : 'Staff'}}</span>
		<span class="text-muted">Reporting Time: {{ $morning_time }}</span>
	</div>
	@endif
@else
	<div>
